...
## 예정 작업
- **관리자 모바일 — 안 타본 플로우 마저 볼 것**: 곡 재생(하단 미리듣기 플레이어 `md:left-64`가 의심), 수정 모달(편집 모드는 벅스 검색이 없는 분기), 삭제 확인, 유튜브 미리듣기. 상세는 아래 2026-07-17 (3) 항목
- **(감시) 검색은 인덱스를 못 탄다 — 곡이 더 늘면 재검토**: `searchSong`의 `createFlexibleRegex`가 글자 사이에 `\s*`를 끼워넣어(`"아이유"` → `/아\s*이\s*유/i`) **인덱스를 못 쓰고 풀스캔**한다. 실측 2026-07-17(13,723곡): "아이유" 404ms / "사랑"(가사 전문) 224ms — **지금은 문제 없다.** 다만 곡 수에 선형이라 여기서 또 10배가 되면 체감된다. 그때 텍스트 인덱스 검토. (성능 문제가 없는데 지금 인덱스부터 넣는 건 이른 최적화)
...
**변경**(`server/services/songService.js` `searchSong`, `type==='all'` 블록만):
- 쿼리를 공백 토큰으로 쪼개 **토큰 간 `$and`**, 각 토큰은 `title $or artist`.
- **단일 토큰이면 `$and:[{$or:[…]}]`라 종전 `$or`과 결과 완전 동일** → 회귀 없음.
- 앱 변경 0(이미 `type=all` 전송). 웹 변경 0(웹 검색은 `activeTab`=title/artist/lyrics 단일 필드라 `all` 안 씀). 관리자(`MusicManagement.vue`)의 "전체"도 같은 `all`이라 동일하게 개선됨.

**검증**(운영 DB `dibe2`, `connectToMongoDB`, 읽기 전용 임시 스크립트):
- `"아이유 좋은날"` → **total=1 "좋은 날 — 아이유(IU)"** ✅ (순서 바꿔 `"좋은날 아이유"`도 동일)
- `"아이유"` 단일 → total=70(종전 동일), `"좋은날"` → 8, `"좋은 날"`(공백) → 10 — 정상

**주의**: `createFlexibleRegex`를 토큰별로 그대로 쓰므로 **인덱스 풀스캔 성질은 그대로**다(위 "감시" 항목 유효). 토큰 수만큼 `$or`가 늘지만 현재 곡 수(13,723)에선 체감 없음.

### 2026-07-17 (3) - 관리자/검색 모바일 반응형 (실기기 프로파일로 플로우 검증)

**배경**: 곡이 13,723개가 되면서 관리자 음원관리를 모바일에서 쓰기가 어려워짐. 사용자가 모바일을 주로 씀.

**🔴 교훈: 정적 스크린샷으로 "잘 되어 있음"이라 판단했다가 사용자 지적으로 뒤집혔다.**
스크롤/터치/플로우를 실제로 타보니 문제가 줄줄이 나왔다. Playwright `devices['iPhone 12']` 로 재현.

| 사용자 지적 | 실제 원인 | 수정 |
|---|---|---|
| 검색에서 스크롤하면 이상함 | **`body` 스크롤 잠금 없음** → 결과를 넘기려는데 뒤 차트가 같이 밀림 | 모달 열릴 때 `body overflow hidden` (+ `beforeDestroy` 해제) |
| 검색 팝업이 화면 넘어감 | `h-[calc(100vh-300px)]` **매직넘버** + 모바일 `100vh` 는 주소창 포함 → 실제보다 큼 | `max-h-full` + `flex-col` + `flex-1 min-h-0` |
| 관리자 유튜브 검색 깨짐 | 결과가 **화면 밖**에 떠서 눌러도 반응 없어 보임 + 중첩 스크롤 + **hover 전용 재생버튼** | `scrollIntoView` + `max-h-none`(모바일) + `opacity-100 sm:opacity-0` |

**추가로 찾아 고친 것**
- **헤더 돋보기 팝업이 화면 정중앙에 흰 박스로 떴다**(`AppHeader.vue`)
  - 위치: `flex items-center` → 돋보기를 눌렀는데 화면 한가운데 떠서 **어디서 나온 팝업인지 연결이 안 됨** → `items-start pt-20` 으로 헤더 아래에 붙임
  - 색: 앱 전체가 다크인데 **이 팝업만 `bg-white` + `bg-gray-100`** 이라 심하게 튐. 데스크탑 검색창은 헤더 안 반투명(`bg-white bg-opacity-20 text-white`)으로 잘 녹아 있었는데 모바일만 밝은 테마였다 → `bg-gray-800`/`bg-gray-700` 로 `SearchResultsModal` 과 톤 통일
  - 검색/닫기 버튼이 아이콘 크기 그대로라 작았음 → 44px
- **하단 플레이어가 검색 모달을 덮었다**: `MusicPlayer` 가 `z-50` 이고 DOM 상 뒤라 같은 `z-50` 모달이 짐 → 모달 `z-[60]`
- **카드 그리드가 모바일 목록으로 부적합**: 커버(`h-48`)가 풀폭으로 늘어나 **한 곡이 화면을 통째로 먹음**. `flex sm:block` 한 벌로 모바일=리스트 행 / 데스크탑=카드 유지
  - 실측: **8,542px(≈10화면) → 4,144px, 화면당 1곡 미만 → 3.9곡**
- **`coverUrl` 없으면 터지는 버그**: `song.coverUrl.replace(...) || '/default...'` — **폴백이 `.replace` 뒤에 있어서 아무 소용이 없었다**(null 이면 replace 에서 이미 터짐). `coverUrl(song, size)` 헬퍼로 정리
- 모달: 모바일 풀스크린 + 제목 sticky + **저장/취소 하단 sticky**(폼이 길어 매번 끝까지 스크롤해야 했음)
- 검색 폼: 모바일에서 [검색바] / [필터·검색·+] 두 줄로 압축(기존 3줄), 터치영역 44px 확보
- 툴팁(`group-hover`)은 `hidden sm:block` — 터치 기기에선 어차피 안 뜬다

**검색 결과 모달의 리스트 자체는 원래 잘 되어 있었다** — 리스트 패턴, 버튼은 아이콘만(`hidden sm:inline`), 터치영역 충분. 위 스크롤/`z-index`/높이 문제만 고침.

**⚠️ 아직 모바일에서 안 타본 플로우** (다음에 이어서 볼 것 — 사용자가 "더 눌러보면 어색한 게 더 보인다"고 함)
- 관리자에서 **곡 재생** → 하단 미리듣기 플레이어 + seek bar. `md:left-64`(사이드바 기준)가 붙어 있어 모바일 레이아웃이 의심스럽다
- **수정** 모달 — 편집 모드는 벅스 검색 영역이 사라지는 분기라 레이아웃이 다르다
- **삭제** 확인 흐름
- 유튜브 결과에서 **미리듣기 재생**(모달 안에 플레이어가 뜨는 부분)

### 2026-07-17 (2) - 제목 오염 파서 수정 + 19금 플래그 + 가사 lazy fill, **DB 적재 완료 (1,091 → 13,723곡)**

> ✅ **파이프라인 완주.** 2026-07-16에 시작한 대량 크롤링이 여기서 DB 적재까지 끝났다.
> 남은 것은 없다 — 아래 "남은 일"은 전부 완료/취소 상태로 기록만 남긴다.
...
4. ~~7,373곡 선별~~ ❌ **취소 — 전량 적재로 결정 (사용자 승인)**
   - 원안은 "2회 이상 + 1회 중 30위 이내 ≈ 7,373곡"이었으나, **가사 lazy fill 이 생기면서 곡을 늘리는 비용이 내려갔다**(원래 걸림돌은 곡이 늘수록 가사 크론 부담이 커지는 것이었는데, 이제 사용자가 튼 곡만 채운다). 장르는 12,632곡 전부 이미 보유 → **12,632곡 전량 적재**
5. ~~DB 적재~~ ✅ **완료** (아래 "작업 5 상세")
6. 유튜브는 급하지 않음 — lazy fill이 재생 시점에 채움

**작업 5 상세 (DB 적재)** — `scripts/load-songs.js` (dry-run 기본, `--apply`)

**DB 1,091 → 13,723곡.** 검증이 전부 교차로 맞음:

| 항목 | 값 | 검증 |
|---|---|---|
| 전체 곡 | **13,723** | 1,091 + 12,632 |
| `adult:true` | **90** | 기존 3 + 신규 87 = 실측 유니크 19금 90 |
| `genre` 있음 | **13,723 (전부)** | 장르 못 찾은 20곡도 `[]`로 닫아서 넣음 → 크론이 재시도 안 함 |
| `youtubeUrl` | 2,187 | 기존 1,091 + 신규 1,096 |
| `lastChartedAt` | 12,632 | 적재분 전부 |
| 오염 제목 / 중복 키 | **0 / 0** | |

- **`lastChartedAt` 은 고정값이 아니라 `songs.jsonl` 의 `lastSeen`(실제 마지막 차트 등장월)을 넣었다.** 원안(고정값)은 `createdAt`(삽입 시각 = 의미 없는 값) 기준으로 논의할 때 얘기였고, 실제 등장월이 이미 있으므로 그게 낫다 — 고정하면 12,632곡이 전부 "방금 차트에 올랐다"가 되어 **적재분 내부에 순서가 사라진다**. 대신 같은 달 곡이 무더기라 **`chartHits` 2차 정렬 + 복합 인덱스**가 필요해졌다
- ⚠️ 적재 스크립트는 `aggregate` 의 `existing` 을 믿지 않고 **실행 시점에 DB를 다시 조회**한다(그 사이 차트 크론이 곡을 더 넣었을 수 있다 — 실제로 이날 08:08 에 선미 "Forever July" 가 들어왔다)
- ✅ **크론 정렬 실측** — 적재 직후 가사 크론이 집는 순서: 마크툽(238회) → 태연 사계(208회) → 잔나비(206회). **최근 차트 인기곡부터** 정확히 잡는다. 정렬이 없었다면 12,632곡이 삽입 순서대로 처리되며 신곡이 뒤로 밀렸을 것
- 롤백 수단: 이번에 `timestamps: true` 를 켰으므로 적재분은 `createdAt` 으로 식별된다 (`deleteMany({ createdAt: { $gte: 적재시각 } })`)

**적재 후 자연히 따라오는 것들** (의도된 동작)
- 11,536곡이 `youtubeUrl` 없이 들어감 → 첫 재생 때 lazy fill(1.1초)
- 가사는 12,632곡 전부 비어 있음 → 재생할 때마다 벅스 요청 1회(**그 곡 최초 1회뿐**)
- 크론은 `{lastChartedAt: -1, chartHits: -1}` 순으로 최근 인기곡부터 갉아먹음
...
| `lastChartedAt` | 크론 우선순위 정렬 키 (+ **복합** 인덱스 `{lastChartedAt: -1, chartHits: -1}`) |
