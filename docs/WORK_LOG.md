...
2. 문제 3: ~~DB 4곡 제목 정리~~ ✅ 완료 → **수집본 정리 → 재집계** (차단 해제됨 — enrich 100% 끝나서 `songs-enriched.jsonl` 에 쓰는 프로세스 없음)
   - 정리 대상에 `songs-enriched.jsonl`도 포함할 것(제목이 키라서 어긋나면 enrich 결과를 재사용 못 함)
   - 정리 = `^\[[^\]]+\]\n` 제거 + 같은 값을 `adult` 플래그로 이관
   - 수집본에도 같은 중복이 있을 수 있다(`[권리없는 곡]\nExtraL` vs `ExtraL`) → 재집계가 `{title,artist}` 로 다시 합치므로 자동 해소되지만, **어느 행이 대표가 되는지** 확인할 것
...
| `songs.jsonl` | 12,635곡 — 인기순(`chartHits`) 정렬 |
| `songs-enriched.jsonl` | **12,635곡 완료(100%)** — 2026-07-17 (2) 세션에서 마무리. 장르 못 찾은 곡 16 |
